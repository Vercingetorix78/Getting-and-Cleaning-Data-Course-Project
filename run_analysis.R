...
relevantColumns <- grep("mean|std", names(mergedSet))
mergedSet <- mergedSet[, relevantColumns]

# Make the variable names of the merged set more readable by elminating the dashes.
names(mergedSet) <- gsub('[-()]', '', names(mergedSet))

# Create a column for the activities. We will treat this column as a factor variable with
...
Activity <- factor(c(trainingActivities, testActivities), 
                     levels = c("1", "2", "3", "4", "5", "6"),
                     labels = c("Walking", "Walking Upstairs", "Walking Downstairs", "Sitting", "Standing", "Laying"))
mergedSet <- cbind(Activity, mergedSet)
...
Tidy <- aggregate(mergedSet[3:81], by = list(Subject, Activity), FUN = mean, na.rm = TRUE)
